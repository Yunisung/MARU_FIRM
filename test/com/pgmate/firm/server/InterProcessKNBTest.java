...
public class InterProcessKNBTest {

    InterProcess interProcess;
    Firm firm;
    private static Logger logger = LoggerFactory.getLogger(InterProcessKNBTest.class);
...
//        comm(firmBean);
    ...
    public void withdrawAccountUnReg() {
        ...
        firmBean.msgType 	= "0900400";
        firmBean.userId		= "SYSTEM";
        firmBean.data.put("trxType", "3");         // (거래구분) ‘1’:신규’ ‘2’:변경, ‘3’:해지
        firmBean.data.put("virtualAccount", "8008308820139");  // (가상계좌번호)
        firmBean.data.put("withdrawBankCd", "230");  // (출금은행코드) PG_CODE 테이블 참조
        firmBean.data.put("withdrawAccount", "144414442800"); // (출금계좌번호)
        firmBean.data.put("customerName", "안영준");    // (고객명)
        firmBean.data.put("companyCd", "MBR00246");        // (필수)
        firmBean.data.put("identity", "7911121");        // (생년월일 + 성별)
        firmBean.data.put("regType", "1");              // (등록유형) 1:개인, 2:법인, 3:미성년자, 4:외국인
